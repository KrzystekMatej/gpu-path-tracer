...
namespace Core::Import
{
	namespace 
	{
		OIIO::TypeDesc GetOiioTypeDesc(Graphics::ComponentType componentType)
		{
			switch (componentType)
			{
				case Graphics::ComponentType::UInt8:
					return OIIO::TypeDesc::UINT8;
				case Graphics::ComponentType::Float16:
		...
		std::string GetOiioColorSpace(Graphics::ColorSpace colorSpace)
		{
			switch (colorSpace)
			{
				case Graphics::ColorSpace::Linear:
					return "Linear";
				case Graphics::ColorSpace::SRGB:
					return "sRGB";
			}
	...
	std::expected<void, Utils::Error> SaveImage(const std::filesystem::path& path, const Image& image)
	{
		if (path.empty())
			return std::unexpected(Utils::Error("Failed to save image, file path is empty"));

		if (path.extension().empty())
			return std::unexpected(Utils::Error("Failed to save image, file extension is missing, file path: {}", path.string()));

		if (image.width == 0 || image.height == 0)
		{
			return std::unexpected(Utils::Error(
				"Failed to save image, invalid dimensions {}x{}, file path: {}",
				image.width,
				image.height,
				path.string()));
		}
		...
		OIIO::ImageSpec spec(static_cast<int>(image.width), static_cast<int>(image.height), static_cast<int>(image.format.GetChannelCount()), outputType);
		spec.attribute("oiio:ColorSpace", GetOiioColorSpace(image.format.colorSpace));

		if (!imageOutput->open(path.string(), spec))
			return std::unexpected(Utils::Error("{}, file path: {}", imageOutput->geterror(), path.string()));

		if (!imageOutput->write_image(outputType, image.data.data()))
		{
			const std::string error = imageOutput->geterror();
			imageOutput->close();
			return std::unexpected(Utils::Error("{}, file path: {}", error, path.string()));
		}
